...
export default function Present() {
  ...
  const bombSlide = React.useMemo(() => {
    const candidates = allSlides.filter(s => s.btbEasy || s.btbHard || s.btbChain || s.btbChainData)
  ...
  function pushToStudent(si, qi, ans) {
    if (si >= allSlides.length) {
      ...
      channel.postMessage({ type: 'slide', isBomb: true, showAns: ans, btbSecs,
        ...
        btbChainData: last?.btbChainData || null,
        skill: last?.skill?.skill_name || '',
        btbMode: window.__dmr_btbMode || 'chain' })
      return
    }
...
function BombSlide({ left, btbSecs, slide, showAns }) {
  ...
  const chainData = slide?.btbChainData || null

  // Content mode — just show two questions
  if (btbMode === 'content') {
    return (
      <div style={{ width:'100%', maxWidth:1200, flex:1, display:'flex', flexDirection:'column', gap:10 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(18px,3vw,38px)', fontWeight:800, color:'#f87171', textAlign:'center', flexShrink:0 }}>💣 BEAT THE BOMB 💣</div>
        <div style={{ fontFamily:'monospace', fontSize:'clamp(44px,8vw,90px)', fontWeight:800, textAlign:'center', lineHeight:1, color:urgent?'#f87171':'#a78bfa', animation:urgent?'pulse .4s infinite':'none', flexShrink:0 }}>
          {left<=0?'💥':left}
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, flex:1, minHeight:0 }}>
          {[
            ...
            <div key={s.l} style={{ background:s.bg, border:`2px solid ${s.b}`, borderRadius:16, padding:'18px 22px', display:'flex', flexDirection:'column' }}>
              <div style={{ fontFamily:"'Syne',sans-serif", fontSize:10, fontWeight:700, letterSpacing:'.14em', textTransform:'uppercase', color:s.c, marginBottom:10 }}>{s.l}</div>
              <div style={{ color:'#e0e7ff', fontFamily:"'JetBrains Mono',monospace", fontSize:'clamp(14px,2vw,26px)', lineHeight:1.6, flex:1, whiteSpace:'pre-wrap' }}>
                {s.q || <span style={{ color:'rgba(255,255,255,.3)', fontStyle:'italic' }}>No question set</span>}
              </div>
              {showAns && <div style={{ marginTop:8, paddingTop:8, borderTop:'1px solid rgba(255,255,255,.15)', fontSize:12, color:'rgba(224,231,255,.5)' }}>Discuss →</div>}
            </div>
          ))}
        </div>
      </div>
  ...
  // Chain mode — step-by-step columns
  // If no structured chain data, show fallback message
  if (!chainData?.normal?.length) {
    return (
      <div style={{ width:'100%', flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(18px,3vw,38px)', fontWeight:800, color:'#f87171' }}>💣 BEAT THE BOMB 💣</div>
        <div style={{ fontFamily:'monospace', fontSize:'clamp(44px,8vw,90px)', fontWeight:800, color:urgent?'#f87171':'#a78bfa', animation:urgent?'pulse .4s infinite':'none' }}>
          {left<=0?'💥':left}
        </div>
        <div style={{ color:'rgba(224,231,255,.4)', fontSize:14 }}>Regenerate slides to load chain data</div>
      </div>
    )
  }

  const { start, normal, challenge } = chainData

  return (
    <div style={{ width:'100%', maxWidth:1320, flex:1, display:'flex', flexDirection:'column', minHeight:0, gap:8 }}>

      {/* Header with countdown */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, gap:10 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:10, fontWeight:800, letterSpacing:'.16em', textTransform:'uppercase', color:'#34d399', padding:'4px 12px', background:'rgba(52,211,153,.1)', border:'1px solid rgba(52,211,153,.3)', borderRadius:6 }}>
          NORMAL
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(13px,1.6vw,18px)', fontWeight:800, color:'#f87171', letterSpacing:'.1em', textTransform:'uppercase' }}>💣 BEAT THE BOMB</div>
          <div style={{ fontFamily:'monospace', fontSize:'clamp(28px,4.5vw,52px)', fontWeight:800, minWidth:72, textAlign:'center', color:urgent?'#f87171':'#a78bfa', animation:urgent?'pulse .4s infinite':'none' }}>
            {left<=0?'💥':left}
          </div>
        </div>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:10, fontWeight:800, letterSpacing:'.16em', textTransform:'uppercase', color:'#f87171', padding:'4px 12px', background:'rgba(240,74,107,.1)', border:'1px solid rgba(240,74,107,.3)', borderRadius:6 }}>
          CHALLENGE
        </div>
      </div>

      {/* Starting number — same for both */}
      <div style={{ display:'flex', justifyContent:'center', flexShrink:0 }}>
        <div style={{ background:'#059669', color:'white', fontFamily:'monospace', fontWeight:800, fontSize:'clamp(20px,3vw,36px)', padding:'8px 32px', borderRadius:10, letterSpacing:'.04em' }}>
          Start: {String(start)}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, flex:1, minHeight:0 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
          {normal.map((step, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:8, flex:1 }}>
              {/* Operation */}
              <div style={{ flex:1, background:'white', border:'2px solid #cbd5e1', borderRadius:8,
                padding:'6px 12px', textAlign:'center', fontFamily:"'JetBrains Mono',monospace",
                fontSize:'clamp(13px,1.6vw,20px)', fontWeight:500, color:'#1e293b',
                display:'flex', alignItems:'center', justifyContent:'center', minHeight:42 }}>
                {step.op}
              </div>
              {/* Running total — hidden until Reveal */}
              <div style={{ minWidth:54, textAlign:'right', fontFamily:'monospace',
                fontSize:'clamp(14px,1.8vw,22px)', fontWeight:700,
                color: showAns ? '#a3e635' : 'rgba(255,255,255,.15)',
                background: showAns ? 'transparent' : 'rgba(255,255,255,.08)',
                borderRadius: showAns ? 0 : 6, padding: showAns ? 0 : '4px 8px',
                transition:'all .3s' }}>
                {showAns ? String(step.result) : '?'}
              </div>
            </div>
          ))}
          {/* Final answer */}
          <div style={{ display:'flex', justifyContent:'flex-start', marginTop:4, flexShrink:0 }}>
            <div style={{ background: showAns ? '#ef4444' : 'rgba(239,68,68,.3)', color:'white',
              fontFamily:'monospace', fontWeight:800, fontSize:'clamp(18px,2.5vw,32px)',
              padding:'8px 24px', borderRadius:10, minWidth:90, textAlign:'center',
              transition:'all .3s', border:'2px solid #ef4444' }}>
              {showAns ? String(normal[normal.length-1]?.result ?? '?') : '?'}
            </div>
          </div>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
          {challenge.map((step, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:8, flex:1, flexDirection:'row-reverse' }}>
              {/* Operation */}
              <div style={{ flex:1, background:'white', border:'2px solid #cbd5e1', borderRadius:8,
                padding:'6px 12px', textAlign:'center', fontFamily:"'JetBrains Mono',monospace",
                fontSize:'clamp(13px,1.6vw,20px)', fontWeight:500, color:'#1e293b',
              ...
              {/* Running total — hidden until Reveal */}
              <div style={{ minWidth:54, textAlign:'left', fontFamily:'monospace',
                fontSize:'clamp(14px,1.8vw,22px)', fontWeight:700,
                color: showAns ? '#a3e635' : 'rgba(255,255,255,.15)',
                background: showAns ? 'transparent' : 'rgba(255,255,255,.08)',
                borderRadius: showAns ? 0 : 6, padding: showAns ? 0 : '4px 8px',
          ...
          <div style={{ display:'flex', justifyContent:'flex-end', marginTop:4, flexShrink:0 }}>
            <div style={{ background: showAns ? '#06b6d4' : 'rgba(6,182,212,.3)', color: showAns ? '#0f172a' : 'white',
              fontFamily:'monospace', fontWeight:800, fontSize:'clamp(18px,2.5vw,32px)',
              padding:'8px 24px', borderRadius:10, minWidth:90, textAlign:'center',
              transition:'all .3s', border:'2px solid #06b6d4' }}>
              {showAns ? String(challenge[challenge.length-1]?.result ?? '?') : '?'}
            </div>
          </div>
        </div>
      </div>
