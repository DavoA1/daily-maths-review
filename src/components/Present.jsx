...
function BombSlide({ left, btbSecs, slide, showAns }) {
  ...
  // Get chain data — new structured format
  const chainData = slide?.btbChainData || null
  const hasChain = chainData && chainData.normal?.length

  // Content mode: just show easy/hard questions
  if (btbMode === 'content') {
    return (
      <div style={{ width:'100%', maxWidth:1200, flex:1, display:'flex', flexDirection:'column', gap:12 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(20px,3.5vw,44px)', fontWeight:800, color:'#f87171', textAlign:'center', flexShrink:0 }}>💣 BEAT THE BOMB 💣</div>
        <div style={{ fontFamily:'monospace', fontSize:'clamp(48px,9vw,100px)', fontWeight:800, textAlign:'center', lineHeight:1, color:urgent?'#f87171':'#a78bfa', animation:urgent?'pulse .4s infinite':'none', flexShrink:0 }}>
          {left<=0?'💥':'⏱ '+left+'s'}
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, flex:1, minHeight:0 }}>
          {[
            ...
            <div key={s.l} style={{ background:s.bg, border:`2px solid ${s.b}`, borderRadius:16, padding:'20px 24px', display:'flex', flexDirection:'column' }}>
              ...
              <div style={{ color:'#e0e7ff', fontFamily:"'JetBrains Mono',monospace", fontSize:'clamp(15px,2.2vw,28px)', lineHeight:1.6, flex:1, whiteSpace:'pre-wrap' }}>
  ...
  // Chain mode: step-by-step columns matching classroom BtB format
  if (!hasChain) {
    // Fallback: no chain data yet
    return (
      <div style={{ width:'100%', maxWidth:1200, flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:20 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(20px,3.5vw,44px)', fontWeight:800, color:'#f87171', textAlign:'center' }}>💣 BEAT THE BOMB 💣</div>
        <div style={{ fontFamily:'monospace', fontSize:'clamp(48px,9vw,100px)', fontWeight:800, color:urgent?'#f87171':'#a78bfa', animation:urgent?'pulse .4s infinite':'none' }}>
          {left<=0?'💥':left}
        </div>
        <div style={{ color:'rgba(224,231,255,.4)', fontSize:14, fontStyle:'italic' }}>Chain challenge will appear here after generating slides</div>
      </div>
    )
  }

  const { start, normal, challenge } = chainData
  const maxRows = Math.max(normal.length, challenge.length)

  return (
    <div style={{ width:'100%', maxWidth:1300, flex:1, display:'flex', flexDirection:'column', minHeight:0 }}>

      {/* Title + timer */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:20, marginBottom:10, flexShrink:0 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(14px,2vw,22px)', fontWeight:800, color:'#f87171', letterSpacing:'.1em', textTransform:'uppercase' }}>💣 Beat the Bomb</div>
        <div style={{ fontFamily:'monospace', fontSize:'clamp(28px,5vw,56px)', fontWeight:800, minWidth:80, textAlign:'center',
          color:urgent?'#f87171':'#a78bfa', animation:urgent?'pulse .4s infinite':'none' }}>
          {left<=0?'💥':left}
        </div>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(14px,2vw,22px)', fontWeight:800, color:'#f87171', letterSpacing:'.1em', textTransform:'uppercase' }}>Bomb 💣</div>
      </div>

      {/* Three-column layout: Normal | Bomb centre | Challenge */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 0px 1fr', gap:0, flex:1, minHeight:0, alignItems:'start' }}>

        {/* NORMAL column */}
        <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
          <div style={{ textAlign:'center', fontFamily:"'Syne',sans-serif", fontSize:11, fontWeight:800,
            letterSpacing:'.16em', textTransform:'uppercase', color:'#34d399', marginBottom:8, padding:'6px',
            background:'rgba(52,211,153,.12)', borderRadius:8, border:'1px solid rgba(52,211,153,.3)' }}>
            NORMAL
          </div>

          {/* Starting number */}
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', marginBottom:6 }}>
            <div style={{ background:'#059669', color:'white', fontFamily:'monospace', fontWeight:800,
              fontSize:'clamp(18px,2.5vw,32px)', padding:'8px 24px', borderRadius:10, minWidth:80, textAlign:'center' }}>
              {String(start)}
            </div>
          </div>
          ...
          {normal.map((step, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:5 }}>
              {/* Operation box */}
              <div style={{ flex:1, background:'white', border:'2px solid #e2e8f0', borderRadius:8,
                padding:'8px 12px', textAlign:'center', fontFamily:"'JetBrains Mono',monospace",
                fontSize:'clamp(13px,1.8vw,20px)', fontWeight:500, color:'#1e293b', minHeight:44,
                display:'flex', alignItems:'center', justifyContent:'center' }}>
                {step.op}
              </div>
              {/* Running total */}
              <div style={{ minWidth:48, textAlign:'right', fontFamily:'monospace',
                fontSize:'clamp(14px,2vw,24px)', fontWeight:700, color:'#e0e7ff' }}>
                {showAns || i === normal.length - 1 ? String(step.result) : '?'}
              </div>
            </div>
          ))}
          ...
          <div style={{ display:'flex', justifyContent:'flex-start', marginTop:4 }}>
            <div style={{ background:'#f87171', color:'white', fontFamily:'monospace', fontWeight:800,
              fontSize:'clamp(18px,2.5vw,32px)', padding:'8px 24px', borderRadius:10, minWidth:80, textAlign:'center' }}>
        ...
        {/* Centre spacer — bomb icon */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', fontSize:'clamp(32px,5vw,64px)',
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
          <div style={{ textAlign:'center', fontFamily:"'Syne',sans-serif", fontSize:11, fontWeight:800,
            letterSpacing:'.16em', textTransform:'uppercase', color:'#f87171', marginBottom:8, padding:'6px',
            background:'rgba(240,74,107,.12)', borderRadius:8, border:'1px solid rgba(240,74,107,.3)' }}>
            CHALLENGE
          </div>
          ...
          {challenge.map((step, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:5 }}>
              {/* Running total */}
              <div style={{ minWidth:48, textAlign:'left', fontFamily:'monospace',
                fontSize:'clamp(14px,2vw,24px)', fontWeight:700, color:'#e0e7ff' }}>
                {showAns || i === challenge.length - 1 ? String(step.result) : '?'}
              </div>
              {/* Operation box */}
              <div style={{ flex:1, background:'white', border:'2px solid #e2e8f0', borderRadius:8,
                padding:'8px 12px', textAlign:'center', fontFamily:"'JetBrains Mono',monospace",
          ...
          <div style={{ display:'flex', justifyContent:'flex-end', marginTop:4 }}>
            <div style={{ background:'#4ac8f0', color:'#0f172a', fontFamily:'monospace', fontWeight:800,
              fontSize:'clamp(18px,2.5vw,32px)', padding:'8px 24px', borderRadius:10, minWidth:80, textAlign:'center' }}>
